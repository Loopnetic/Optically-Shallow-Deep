...
from .cloud_mask import cloud_mask, cloud_mask_L1C
import rasterio

def run_L2R(file_L2R, folder_out, to_log=True):
    ...
    # make cloud mask 
    img_cloud = cloud_mask(image_path)
    #Save cloud mask for debugging
    with rasterio.open(image_path) as src:
        profile = src.profile
        profile.update(dtype=rasterio.uint8, count=1, compress='lzw', nodata=None)
        cloud_mask_path = image_path.replace('.tif','_cloud_mask.tif')
        with rasterio.open(cloud_mask_path, 'w', **profile) as dst:
            dst.write(img_cloud.astype(rasterio.uint8), 1)
...
def run_L1C(file_L1C, folder_out, file_L2R = None, to_log=True):
    ...
    if if_SR:
        ...
        img_cloud = cloud_mask_L1C(file_L1C)
